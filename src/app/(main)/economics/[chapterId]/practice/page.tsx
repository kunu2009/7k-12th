...
import { getEconomicsQuestionsByChapter, getEconomicsQuestionsByType, type EconomicsQuestion } from '@/lib/data/economics/question-bank';
...
export default function PracticePage({ params }: PracticePageProps) {
  ...
  const allQuestions = getEconomicsQuestionsByChapter(chapterId);

  const [selectedType, setSelectedType] = useState<EconomicsQuestion['type']>('2-marks');
  ...
  const filteredQuestions = getEconomicsQuestionsByType(chapterId, selectedType);
  ...
  const handleTypeChange = (type: EconomicsQuestion['type']) => {
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['1-mark', '2-marks', '4-marks', '6-marks'].map((type) => {
              const count = getEconomicsQuestionsByType(chapterId, type as EconomicsQuestion['type']).length;
              const completed = getEconomicsQuestionsByType(chapterId, type as EconomicsQuestion['type']).filter(
              ...
              return (
                <Button
                  ...
                  onClick={() => handleTypeChange(type as EconomicsQuestion['type'])}
