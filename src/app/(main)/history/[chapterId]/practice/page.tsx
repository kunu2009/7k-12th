...
import { getHistoryQuestionsByChapter, getHistoryQuestionsByType, type HistoryQuestion } from '@/lib/data/history/question-bank';
...
export default function PracticePage({ params }: PracticePageProps) {
  ...
  const allQuestions = getHistoryQuestionsByChapter(chapterId);

  const [selectedType, setSelectedType] = useState<HistoryQuestion['type']>('2-marks');
  ...
  const filteredQuestions = getHistoryQuestionsByType(chapterId, selectedType);
  ...
  const handleTypeChange = (type: HistoryQuestion['type']) => {
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['1-mark', '2-marks', '4-marks', '6-marks'].map((type) => {
              const count = getHistoryQuestionsByType(chapterId, type as HistoryQuestion['type']).length;
              const completed = getHistoryQuestionsByType(chapterId, type as HistoryQuestion['type']).filter(
              ...
              return (
                <Button
                  ...
                  onClick={() => handleTypeChange(type as HistoryQuestion['type'])}
